...
| Backend API | ✅ M0+M2 done, M3 done | `/health`, `/restaurants`, `/auth/register`, `/auth/login` (JWT bearer), `/preferences`, `/recommend`, `/chat`, `/history` (+`/stats`) + CORS + error envelope |
| API contract | ✅ Committed | `docs/api/openapi.yaml` + fixtures, generated by `backend/tools/make_contract.py`; auth fixtures via register/login (`register.json`) |
| Backend tests | ✅ Running | `backend/tests/` — 124 passing, 1 opt-in live-LLM skip, on a throwaway DB (`pytest` from `backend/`) |
...
| Chat (M2) | ✅ BE done | `POST /chat` + `backend/app/llm/` (extract → filter → narrate); tests in `tests/test_chat.py`; `chat_*.json` fixtures |
| Places, history | ✅ Done | Places enrichment on restaurant detail; `POST/GET /history` behind bearer auth with Flutter batching (`HistoryBuffer`) |
| Flutter app | ✅ Built | Riverpod screens (chat/profile/detail), models, Dio/http clients, JWT login + secure token storage (`lib/features/auth/`) |
| Flutter deps | ✅ Full | riverpod, dio, http, go_router, geolocator, flutter_secure_storage, uuid |
