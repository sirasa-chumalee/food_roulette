...
| **Source flags contradict dish names** — found at M2: `เล้งธรรมดา`/`โคตรเล้ง` (pork spine soup) and `Beef Burger` are `contains_pork: 0` / `contains_meat: 0` / `is_vegan: true`, all at `confidence: "high"`. A name-vs-flag scan finds 8 such rows in 1250. | A "no pork" or vegan request can return a pork dish, and `confidence: "high"` means the Tier-B caution badge never fires. The filter is correct; its input is not — so no amount of backend logic catches this. | Needs a data pass, not code: re-label from the dish names before any real user sees it. Until then, the `confidence` tier is not trustworthy as a proxy for "we checked this". Do not paper over it in `filter.py`. | **Owner + Backend** |
